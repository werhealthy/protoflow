...
// Declare globals provided by Figma environment
...
function getHotspotsForFrame(
  frame: any,
  allFramesMap: Record<string, any>,
  warnings: any[],
  selectedFrameIds: Set<any>
) {
  const hs: any[] = [];
  ...
  const nodesToCheck = [
    frame,
    ...frame.findAll((n: any) => n.reactions && Array.isArray(n.reactions)),
  ];
...
function notifyUIWithCurrentSelection() {
  ...
  const frames = selection.filter((n: any) => n.type === 'FRAME');
  const selectedFrameIds = new Set(frames.map((f: any) => f.id));
  const allFrames = figma.currentPage.findAll((n: any) => n.type === 'FRAME');
  ...
  figma.ui.postMessage({
    ...
    warnings,
    frames: frames.map((f: any) => ({ id: f.id, name: f.name })),
  });
}
...
figma.ui.onmessage = async (msg: any) => {
  ...
  if (msg.type === 'proceed-export') {
    ...
    const frames = selection.filter((n: any) => n.type === 'FRAME');
    const selectedFrameIds = new Set(frames.map((f: any) => f.id));
    const allFramesMap = getAllFrames(figma.currentPage.findAll((n: any) => n.type === 'FRAME'));

    frames.sort((a: any, b: any) => a.name.localeCompare(b.name));

    const framesExport: any[] = [];
    const htmls: any[] = [];

    const homeFrame = frames.find((f: any) => f.id === selectedHomeId) || frames[0];
    ...
    const homeHtml = htmls.find((h: any) => h.name === `screens/${homeName}.html`);
    if (homeHtml) {
      const fixedContent = homeHtml.content.replace(/src="screens\/([^" ]+)\.png"/g, 'src="screens/$1.png"');
      const rawHomeHtml = htmls.find((h: any) => h.name === `screens/${homeName}.html`);
      if (rawHomeHtml) {
